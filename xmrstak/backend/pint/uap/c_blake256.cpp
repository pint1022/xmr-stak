...
void blake256_update(state* S, uint8_t* data, uint32_t datalen)
{
	int left = S->buflen >> 3;
	int fill = 64 - left;
...
// datalen = number of bits
void blake224_update(state* S,  uint8_t* data, uint32_t datalen)
{
	blake256_update(S, data, datalen);
}

void print_msg(char* msgname, uint8_t *msg, int length) {
	pint_printf("%s:\n", msgname);
...
//	print_msg("msglen", msglen, 8);
//	pint_printf("buflen %d\n", S->buflen);
...
void blake256_hash(uint8_t* out, uint8_t* in, uint32_t inlen)
{
	state S;
	blake256_init(&S);
//	print_state(&S);
...
// inlen = number of bytes
void blake224_hash(uint8_t* out, uint8_t* in, uint32_t inlen)
{
	state S;
	blake224_init(&S);
