...
//		keccakf(hash, 24);
		pint_keccakf((uint64_t*)hash, 25*64, (char*)hash, 24);
...
//show_pint("CN_INIT (inputs)", input + len * n, len);																																			\
...
#define CN_FINALIZE(n)                                                                                                 \
		/* Optim - 99% time boundary */                                                                                    \
		show_pint("CN_FINALIZE (Inputs) ", ctx[n]->hash_state, 200);						\
		pint_keccakf((uint64_t*)ctx[n]->hash_state, 200, (char*)ctx[n]->hash_state, 24);		\
		show_pint("CN_FINALIZE (outputs)", ctx[n]->hash_state, 200);							\
		printf("pint_extra_hashes %d n %d strlen %d\n", ctx[n]->hash_state[0] & 3, n, strlen((char*)ctx[n]->hash_state));		\
...
//			keccakf((uint64_t*)ctx[i]->hash_state, 24);
			pint_keccakf((uint64_t *)ctx[i]->hash_state, 24*64, (char*)ctx[i]->hash_state, 24);																\
			pint_extra_hashes[ctx[i]->hash_state[0] & 3](ctx[i]->hash_state, 200, (char*)output + 32 * i);
		}
	}
};
...
//		printf("%s, %d: \n", __FILE__, __LINE__);
		...
		{
//			keccak((const uint8_t*)input + len * i, len, ctx[i]->hash_state, 200);
			pint_hash((char*)input + len * i, len, ctx[i]->hash_state, OP_KECCAK, 200);
			/* Optim - 99% time boundary */
			cn_explode_scratchpad<false, false, ALGO>((__m128i*)ctx[i]->hash_state, (__m128i*)ctx[i]->long_state, algo);
		}
...
//		keccak((const uint8_t*)input, len, ctx[0]->hash_state, 200);
		pint_hash((char*)input, len, ctx[0]->hash_state, OP_KECCAK_MD, 200);
...
//		pint_keccakf((uint64_t*)ctx[0]->hash_state, 24*64,(char*)ctx[0]->hash_state, 24);																\
