...
HashReturn Update(hashState* state, const BitSequence* data, DataLength databitlen)
{
	...
	if((state->datasize_in_buffer > 0) && ((state->datasize_in_buffer + databitlen) < 512))
	{
		if((databitlen & 7) == 0)
		{
			...
			memcpy(state->buffer + (state->datasize_in_buffer >> 3), (char*)data, 64 - (state->datasize_in_buffer >> 3) + 1);
...
HashReturn Final(hashState* state, BitSequence* hashval)
{
	...
	{
	case 224:
		memcpy(hashval, state->x + 64 + 36, 28);
		break;
	case 256:
		memcpy(hashval, state->x + 64 + 32, 32);
		break;
	case 384:
		memcpy(hashval, state->x + 64 + 16, 48);
		break;
	case 512:
		memcpy(hashval, state->x + 64, 64);
		break;
	}
