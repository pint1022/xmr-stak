...
void print_ctx(groestlHashState* S) {
	int count = SIZE512 / sizeof(uint32_t);
	pint_printf("\nchaining\n");
	for (int i=0; i<8; i++)
		pint_printf("%x ", S->chaining[i]);
	pint_printf("\nbuffer\n");
	for (int i=0; i<SIZE512; i++)
		pint_printf("%x ", S->buffer[i]);
	pint_printf("\nbuf_ptr %d\n ", S->buf_ptr);}
/* initialise context */
...
/* hash bit sequence */
void groestl( BitSequence* data, DataLength databitlen, BitSequence* hashval)
{
