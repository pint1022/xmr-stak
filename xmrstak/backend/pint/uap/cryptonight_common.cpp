...
void do_blake_hash(void* input, uint32_t len, char* output)
{
//	printf("%s %d: \n", __FILE__, __LINE__);
	pint_hash( ( char*)input, len, (char*)output, OP_BLAKE, len);
}

void do_groestl_hash(void* input, uint32_t len, char* output)
{
//	printf("%s %d: \n", __FILE__, __LINE__);
//	groestl(( uint8_t*)input, len * 8, (uint8_t*)output);
	pint_hash( ( char*)input, len, (char*)output, OP_GROESTL, len);
}

void do_jh_hash(void* input, uint32_t len, char* output)
{
//	jh_hash(32 * 8, ( uint8_t*)input, 8 * len, (uint8_t*)output);
//	printf("%s %d: \n", __FILE__, __LINE__);

	pint_hash( (char*)input, len, (char*)output, OP_JH, len);

}

void do_skein_hash(void* input, uint32_t len, char* output)
{
//	skein_hash(8 * 32, ( uint8_t*)input, 8 * len, (uint8_t*)output);
//	printf("%s %d: \n", __FILE__, __LINE__);
	pint_hash( (char*)input, len, (char*)output, OP_SKEIN, len);

}

void (*const pint_extra_hashes[4])(void*, uint32_t, char*) = {do_blake_hash, do_groestl_hash, do_jh_hash, do_skein_hash};
