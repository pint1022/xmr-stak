...
/*
 * Parts of this file are originally copyright (c) 2018, Panyi Inc
 *
 */
#pragma once
...
void pint_soft_aesenc(pint_m128i in, pint_m128i key, pint_m128i out)
{
	uint32_t x0, x1, x2, x3;
	x0 = pint_mm_cvtsi128_si32(in);
	pint_m128i tmp_m;
	pint_mm_shuffle_epi32(in, 0x55, tmp_m);
	x1 = pint_mm_cvtsi128_si32(tmp_m);
	pint_mm_shuffle_epi32(in, 0xAA, tmp_m);
	x2 = pint_mm_cvtsi128_si32(tmp_m);
	pint_mm_shuffle_epi32(in, 0xFF, tmp_m);
	x3 = pint_mm_cvtsi128_si32(tmp_m);
	...
	pint_mm_set_epi32(
		...
		(saes_table[0][x0 & 0xff] ^ saes_table[1][(x1 >> 8) & 0xff] ^ saes_table[2][(x2 >> 16) & 0xff] ^ saes_table[3][x3 >> 24]), tmp_m);

	pint_mm_xor_si128(tmp_m, key, out);
}
...
void pint_soft_aeskeygenassist(pint_m128i key, uint8_t rcon, pint_m128i out)
{
	uint32_t X1 = sub_word(pint_mm_cvtsi128_si32(pint_mm_shuffle_epi32(key, 0x55)));
	uint32_t X3 = sub_word(pint_mm_cvtsi128_si32(pint_mm_shuffle_epi32(key, 0xFF)));
	pint_mm_set_epi32(_rotr(X3, 8) ^ rcon, X3, _rotr(X1, 8) ^ rcon, X1, out);
}
