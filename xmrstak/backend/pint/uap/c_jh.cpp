/* This program gives the 32-bit bitslice reference implementation of JH using ANSI C
   ...
   1) 123.0 cycles/byte   compiler: Intel C++ Compiler 11.1   compilation option: icc -O2
   2) 156.3 cycles/byte   compiler: gcc 4.4.3                 compilation option: gcc -O3
...
/*for LP64 data model, "int" is 32-bit integer, while "long" is 64-bit integer*/
...
typedef struct {
	...
	unsigned int databitlen;    /*the message size in bits*/
...
void RoundFunction(hashState* state, uint32 roundnumber);  /*The round function of E8, in bitslice form*/
...
void RoundFunction(hashState* state, uint32 roundnumber) {
      ...
      {
            case 0:
                  for (j = 1; j < 8; j = j+2) for (i = 0; i < 4; i++) SWAP1(state->x[j][i]);
                  break;
            case 1:
                  for (j = 1; j < 8; j = j+2) for (i = 0; i < 4; i++) SWAP2(state->x[j][i]);
                  break;
            case 2:
                  for (j = 1; j < 8; j = j+2) for (i = 0; i < 4; i++) SWAP4(state->x[j][i]);
                  break;
            case 3:
                  for (j = 1; j < 8; j = j+2) for (i = 0; i < 4; i++) SWAP8(state->x[j][i]);
                  break;
            case 4:
                  for (j = 1; j < 8; j = j+2) for (i = 0; i < 4; i++) SWAP16(state->x[j][i]);
                  break;
            case 5:
...
void E8(hashState *state)
{
      ...
      /*perform 42 rounds*/
      for (i = 0; i < 42; i++) RoundFunction(state, i);
}
...
void F8(hashState *state)
{
      ...
      for (i = 0; i < 16; i++)  state->x[(16+i) >> 2][(16+i) & 3] ^= ((uint32*)state->buffer)[i];
}
...
HashReturn Update(hashState *state, const BitSequence *data, DataLength databitlen)
{
      ...
      if ( (state->datasize_in_buffer > 0 ) && (( state->datasize_in_buffer + databitlen) < 512)  ) {
            if ( (databitlen & 7) == 0 ) {
                 memcpy(state->buffer + (state->datasize_in_buffer >> 3), (char*)data, 64-(state->datasize_in_buffer >> 3)) ;
		    }
            else memcpy(state->buffer + (state->datasize_in_buffer >> 3), (char*)data, 64-(state->datasize_in_buffer >> 3)+1) ;
      ...
      for ( ; databitlen >= 512; index = index+64, databitlen = databitlen - 512) {
            memcpy(state->buffer, (char*)data+index, 64);
            F8(state);
      }
      ...
      if ( databitlen > 0) {
            if ((databitlen & 7) == 0)
                  memcpy(state->buffer, (char*)data+index, (databitlen & 0x1ff) >> 3);
            else
                  memcpy(state->buffer, (char*)data+index, ((databitlen & 0x1ff) >> 3)+1);
            state->datasize_in_buffer = databitlen;
      }
